...
// @Repository
// public interface UserTaskRepository extends ReactiveMongoRepository<UserTask,
// String> {

// @Aggregation(pipeline = { "{$match: {userId: '?0', archived: {$ne : true}
// }}", "{$sort: { sortValue: -1 }}",
// "{$addFields: {taskId: {$toObjectId: \"$taskId\"} }}",
// "{$lookup: {from: \"tasksTest\", localField: \"taskId\", foreignField:
// \"_id\", as: \"result\"}}",
// "{$addFields: {result: {$first: \"$result\"}}}",
// "{$match: {result: {$ne: null}}}", "{$limit: 1}", "{$replaceRoot: {newRoot:
// \"$result\"}}" })
